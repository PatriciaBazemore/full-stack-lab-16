...
app.route('/api/chirps')  ///multiple chirps
    ...
    row('InsertChirp', [newChirp.message, newChirp.user])  //call row, will return id
    .then(function(id) {
...
function empty(procedureaName, args) {    //use for insert, delet--- we return nothing, aka resovle()
    return callProcedure(procedureaName, args)
        .then(function() {
            return;
        });
}